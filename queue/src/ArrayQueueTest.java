...
public class ArrayQueueTest {
    public static void main(String[] args) {
        ...
        ArrayQueueAdapter queue = new ArrayQueueAdapter();
        for(int i=100;i<1000;i++){
            queue.enqueue((Object) (i%256));
        }
        printMinimum(queue);
    ...
    private static void testSingleQueue(Queue queue) {
        ...
        for (int i : new int[]{10, 100, 10000, 100000}) {
    ...
    public static void printMinimum(Queue queue)
    {
        assert queue.size()>0;
        int min = Integer.MAX_VALUE,_min;
        for(int i=0;i<queue.size();i++)
        {
            _min=(Integer) queue.dequeue();
            if(_min< min){
                min=_min;
            }
            queue.enqueue((Object) _min);
        }
